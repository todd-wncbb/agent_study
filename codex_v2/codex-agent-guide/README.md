...
```mermaid
flowchart TD
    UI["CLI / TUI / App Server"] -->|"提交 Op::UserInput"| THREAD["CodexThread"]
...
16. [配置系统与 Feature Flag](16-configuration-and-features.md)：配置层如何合并、约束并派生运行能力。
17. [端到端数据实例](17-end-to-end-data-example.md)：跟踪一条用户输入在每层的具体结构。
18. [安全模型与信任边界](18-security-and-trust-boundaries.md)：Prompt Injection、权限和副作用治理。
19. [协议、事件与 App Server](19-protocol-events-and-app-server.md)：客户端与 Agent Runtime 如何通信。
20. [Hooks 与 Extensions](20-hooks-and-extensions.md)：生命周期扩展、上下文和工具贡献机制。
21. [Code Mode](21-code-mode.md)：模型如何在受控 JavaScript Runtime 中组合工具。
22. [执行环境与 Remote Executor](22-execution-environments.md)：本地、远程、文件系统和环境一致性。
23. [长期 Memory](23-long-term-memory.md)：跨线程记忆与短期 History 的区别。
24. [模型、Provider 与认证](24-models-providers-and-auth.md)：模型发现、能力解析、路由和登录。
25. [Token、预算与性能](25-token-budget-and-performance.md)：上下文、缓存、延迟和资源控制。
...
- [状态机图集](appendices/state-machines.md)
- [错误处理目录](appendices/error-catalog.md)
- [源码阅读练习](appendices/exercises.md)
- [Agent 设计模式](appendices/design-patterns.md)
